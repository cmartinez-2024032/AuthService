...
using AuthService2024032.Application.Exceptions;
using AuthService2024032.Application.Extensions;
...
namespace AuthService2024032.Application.Services;
    ...
    ILogger<AuthService> logger) : IAuthService
{
    ...
    public async Task<RegisterResponseDto> RegisterAsync(RegisterDto registerDto)
    {
        ...
        if (await userRepository.ExistByUsernameAsync(registerDto.Username))
        {
        ...
        {
            profilePicturePath = _cloudinaryService.GetDeFailtAvatarUl();
        }
    ...
    private UserResponseDto MapToUserResponseDto(User user)
    {
        ...
        return new UserResponseDto
        {
            ...
            Email = user.Email,
            SPRofilePicture = _cloudinaryService.GetFullImageUrl(user.UserProfile?.ProfilePicture ?? string.Empty),
    ...
    {
        var user = await userRepository.GetByEmailVerofocationTokenAsync(verifyEmailDto.Token);
    ...
    public async Task<EmailResponseDto> ForgotPasswordAsync(ForgotPasswordDto forgotPasswordDto)
    {
        ...
        if (user.UserPasswordReset == null)
        {
            user.UserPasswordReset = new UserPasswordReset
            {
                ...
                PasswordResetToken = DateTime.UtcNow.AddHours(1)
            };
        }
        else
        {
            user.UserPasswordReset.UserPasswordResetToken = resetToken;
            user.UserPasswordReset.PasswordResetToken = DateTime.UtcNow.AddHours(1); // 1 hora para resetear
        }
    ...
    public Task<EmailResponseDto> ForgotPasswordAsyncc(ForgotPasswordDto forgotPasswordDto)
    {
